...
Mat read_image_specific_res(const String &fileName) {
    ...
    try {
        ...
        if (img.empty()) {
            ESP_LOGE("opencv_tests", "Failed to load image: %s", fileName.c_str());
            exit(1); // Prevent further execution
        }
    } catch (cv::Exception &e) {
        ESP_LOGE("opencv_tests", "Exception: %s", e.msg.c_str());
        exit(1);
    }
...
void benchmarkTask(void *pvParameters) {
    ...
    auto *testImages = new imagesList;
    for (const String &res: images_res) {
        testImages->emplace_back(tuple<string, const Mat &>(res, read_image_specific_res("/spiffs/" + res + ".png")));
        ESP_LOGI(TAG, "next img");
    }
    if (testImages == nullptr) {
        ESP_LOGE("BenchmarkTask", "Null testImages pointer!");
    ...
    cout << summary << endl;
    // Delete task after execution to free up resources
    delete testImages;
    esp_task_wdt_delete(nullptr);
    vTaskDelete(NULL);
}
...
extern "C" void app_main(void) {
    ...
    /* SPIFFS init */
    init_spiffs(); {
        cv::Mat testMat(160, 160, CV_8UC3, Scalar(0));
        if (testMat.empty()) {
            ESP_LOGE("opencv_tests", "OpenCV initialization failed!");
        }
    }
    ...
    xTaskCreatePinnedToCore(
        benchmarkTask, // Task function
        "BenchmarkTask", // Task name
        16384, // Stack size (increase if needed)
        nullptr, // Task parameters (not needed here)
        1, // Task priority (1 is low, increase if needed)
        nullptr, // Task handle (not used here)
        1 // Core ID (0 or 1, depending on your setup)
    );
